...
$(function() {
	...
	if (flag == 0) {
		for (let prop in Data) {
			...
			if (Data[prop].id == id) {
				li += `<li index="${Data[prop].id}" node="${node}"  class="${data.className}">${Data[prop].title}</li>`
			} else {
				li += `<li index="${Data[prop].id}" node="${node}">${Data[prop].title}</li>`
			}
		}
		createPage({
			title: data.title,
		...
		for (let prop in Data) {
			if (Data[prop].id == id) {
				li += `<li index="${Data[prop].id}" class="${data.className}">${Data[prop].e_title}</li>`
			} else {
	...
	$('.search_type ul li:nth-child(1)').addClass(data.className);
	...
	$('.hot_top ul').on('click', 'li', function() {
		...
		console.log(checkPage('fans_details'))
		toPage({
...
function createPage(data) {
	...
	$('.search_type ul').on('click', 'li', function() {
		$('.search_type ul li').removeClass("" + sData.className + "")
		$(this).addClass("" + sData.className + "");
	})
}
...
function findList(page, limit) {
	$.ajax({
		...
		dataType: 'json',
		success: (res) => {
			let result = res.result.sort(compare('hits'))
...
function createElement(arr) {
	...
	$('.fans-questions-list-box .fans-questions-list').css({
		'color': "" + data.color + ""
	})
	$('.fans-questions-list-box .questions-descripts').css({
		'color': "" + data.color + ""
	})
